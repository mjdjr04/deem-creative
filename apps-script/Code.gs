...
// ─── Thank-you emails for externally-created bookings (time-driven) ─────────

/**
 * Hourly trigger. Thanks clients who booked through a channel that DOESN'T go
 * through doPost() above — e.g. Google Appointment Scheduling or manually-added
 * invites. Site bookings are already emailed by doPost, so their event titles
 * are deliberately kept OUT of THANKYOU_TITLES to avoid double-emailing.
 *
 * Idempotent: every emailed event is tagged 'thankYouSent', so a re-run (or a
 * late/overlapping trigger) never double-sends. Because of that guard the
 * lookback window can be generous without risking duplicates.
 *
 * NOTE: keep THANKYOU_TITLES in sync with your Appointment Scheduling event
 * titles. They must NOT overlap the site's booking titles (see doPost / TYPES).
 */
var THANKYOU_TITLES = [
...
function processNewBookings() {
  var now = new Date();
  // 2h window comfortably covers an hourly trigger even if it fires late; the
  // 'thankYouSent' tag prevents any duplicate send from the overlap.
  var lookback = new Date(now.getTime() - 2 * 60 * 60 * 1000);
  var cal = CalendarApp.getDefaultCalendar();

  // This Calendar read is the storage op that threw RESOURCE_EXHAUSTED on
  // 6/30 — retry transient Google backend blips instead of failing the run.
